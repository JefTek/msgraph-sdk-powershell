...
Update-MgOrganizationSettingItemInsight -OrganizationId <String>
 -BodyParameter <IMicrosoftGraphItemInsightsSettings> [-PassThru] [-Confirm] [-WhatIf] [<CommonParameters>]
```
...
```yaml
Type: Microsoft.Graph.PowerShell.Models.IMicrosoftGraphItemInsightsSettings
...
### -DisabledForGroup
The ID of an Azure AD group, of which the members' item insights are disabled.
Default is empty.
Optional.
...
true if organization item insights are enabled; false if organization item insights are disabled for all users without exceptions.
Default is true.
Optional.
...
BODYPARAMETER <IMicrosoftGraphItemInsightsSettings>: itemInsightsSettings
